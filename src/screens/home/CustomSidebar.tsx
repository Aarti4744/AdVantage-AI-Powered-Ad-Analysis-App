  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => navigation.closeDrawer()}><ChevronLeft color="#475569" size={20} /></TouchableOpacity>
      </View>
      ...
      <ScrollView style={styles.menu}>
        {menuItems.map((item) => {
          ...
          return (
            <TouchableOpacity key={item.label} style={[styles.navItem, isActive && styles.activeNav]} onPress={() => navigation.navigate(item.route)}>
        ...
        {loading ? <ActivityIndicator color="#22d3ee" /> : (
          <View style={styles.userCard}>
            ...
            <View>
              ...
              <Text style={styles.userPlan}>Quota Remaining: {user?.quota?.remaining ?? 0}</Text>
            </View>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  signOut: { flexDirection: 'row', alignItems: 'center', gap: 10 },
